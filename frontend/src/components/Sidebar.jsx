          ...
          {WOW_ROLES.map((role) => (
            <div key={role.key} className="role-item">
              <div className="role-left">
                ...
                />
                <span className="role-badge" style={{ background: role.bg, color: role.color }}>{role.name}</span>
              </div>
          ...
          {WOW_CLASSES.map(({ name: className }) => (
            <div key={className} className="class-item">
              <div className="role-left">
                ...
                <span className="player-class" style={{ color: getClassColor(className) }}>{className}</span>
              </div>
      ...
      <div className="sidebar-section">
        ...
        <div className="coverage-category">
          ...
          {buffCoverage.standardBuffs.map((buff) => (
            <div key={buff.name} className={`coverage-item ${buff.covered ? 'covered' : 'missing'}`}>
        ...
        <div className="coverage-category">
          ...
          {buffCoverage.raidUtility.map((utility) => (
            <div key={utility.name} className={`coverage-item ${utility.covered ? 'covered' : 'missing'}`}>
              <span className="check-icon">{utility.covered ? '✅' : '⭕'}</span>
